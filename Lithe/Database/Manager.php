...
class Manager
{
     ...
     */
    private static function createDatabaseIfNotExists(array $dbConfig)
    {
        try {
    ...
    {
        return include __DIR__ . '/../Config/database/settings.php';
    }
    ...
    public static function initialize(string $name = null)
    {
        try {
            $requiredEnvVariables = [
                self::DB_CONNECTION_METHOD, self::DB_CONNECTION, self::DB_HOST,
                self::DB_NAME, self::DB_USERNAME, self::DB_PASSWORD, self::DB_SHOULD_INITIATE
            ];
            ...
            if (isset($settings[$name])) {
                ...
                try {
                    ...
                    die('An error occurred while setting up the database connection.');
                }
            } else {
                die("Database configuration '$name' not found.");
            }
        } catch (Exception $e) {
            // Log detailed error
            Log::error("Error initializing the database: " . $e->getMessage());

            die("An error occurred while initializing the database.");
        }
    }
}
