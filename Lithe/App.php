...
class App extends \Lithe\Http\Router
{
    ...
    private $Options = [
        ...
        'views' => PROJECT_ROOT . '/src/views',  // Specifies the default directory for views
    ];
     ...
     * Request object.
     *
     * Retrieves an instance of the Request object representing the HTTP request.
     * The Request object encapsulates details such as URL parameters, HTTP headers,
     * and request body contents.
     *
     * @return \Lithe\Contracts\Http\Request
     */
    private function Request(): \Lithe\Contracts\Http\Request
    {
        // Imports the Request.php file and returns its result, which should be an instance of \Lithe\Contracts\Http\Request.
        return Import::file(__DIR__ . '/Http/Request.php');
    }
     ...
     */
    private function Response(): \Lithe\Contracts\Http\Response
    {
        ...
        // which should be an instance of \Lithe\Contracts\Http\Response.
     ...
     * @param callable $handler The handler to deal with the HTTP exception. It should accept Lithe\Contracts\Http\Request and \Lithe\Contracts\Http\Response as parameters.
     * @return void
     */
    public function httpException(int $status, callable $handler): void
    {
     ...
     * @param \Lithe\Contracts\Http\Request $request The HTTP request object.
     * @param \Lithe\Contracts\Http\Response $response The HTTP response object.
     * @param mixed $exception Optional. The exception that caused the HTTP error.
     * @return void
     */
    private function handleHttpException(int $statusCode, \Lithe\Contracts\Http\Request $request, \Lithe\Contracts\Http\Response $response, $exception = null): void
    {
    ...
    public function listen()
    {
        ...
        try {
            ...
            $route = $this->matchRouteForRequest($request);
            ...
            if ($route) {
                ...
                throw new \Lithe\Contracts\Http\HttpException(404, 'Not found');
            }
        } catch (\Lithe\Contracts\Http\HttpException $e) {
     ...
     *
     * @param \Lithe\Contracts\Http\Request $request The HTTP request to be matched.
     * @return array|null The matched route or null if no route matches.
     */
    private function matchRouteForRequest(\Lithe\Contracts\Http\Request $request)
    {
        // Iterate through routes to find a match
        foreach ($this->routes as $route) {
            if ($route['method'] === $request->method && $this->matchRoute($route['route'], $request->url)) {
                // Extract parameters from the URL
                $request->params = $this->extractParams($route['route'], $request->url);
                return $route;
            }
        }
        // No route matched
     ...
     * @param \Lithe\Contracts\Http\Request $request The HTTP request.
     * @param \Lithe\Contracts\Http\Response $response The HTTP response.
     * @return void
     */
    private function handleRoute(array $route, \Lithe\Contracts\Http\Request $request, \Lithe\Contracts\Http\Response $response)
    {
        // Run middlewares for the route
        $this->runMiddlewares($this->middlewares, $request, $response, function () use ($route, $request, $response) {
     ...
     *
     * @param \Lithe\Contracts\Http\Response $response The response object to send.
     * @param int $statusCode The HTTP status code for the error.
     * @param string $message The error message to display.
     * @return void
     */
    private function sendErrorPage(\Lithe\Contracts\Http\Response $response, int $statusCode, string $message)
    {
