...
use Lithe\Component\Validator;

return new class implements \Lithe\Contracts\Http\Request
{
    /**
    ...
    private function extractFiles()
    {
        ...
        try {
            ...
            foreach ($_FILES as $key => $fileInfo) {
                // Only process if the $fileInfo is an array (which it should be)
                if (is_array($fileInfo) && isset($fileInfo['name'])) {
                    // Assign each file information to the $files object dynamically
                    $files->$key = new \Lithe\Component\Upload($fileInfo);
                }
            }
    ...
    */
    ...
    public function is(string $url): bool
    {
        ...
        $regexPattern = str_replace('*', '.*', preg_quote($url, '/'));

        // Add delimiters and start/end matching
    ...
    public function validate(array $rules): \Lithe\Component\Validator
    {
        ...
        return new \Lithe\Component\Validator($data, $rules);
    }
    ...
    {
        return new \Lithe\Support\Session;
    }
};
